...
FLAMEGPU_AGENT_FUNCTION(move, flamegpu::MessageSpatial3D, flamegpu::MessageNone) {
    ...
    FLAMEGPU->setVariable<float>("drift", sqrtf(fx*fx + fy*fy + fz*fz));  // todo: runtime conditional via templating?
    return flamegpu::ALIVE;
}
...
FLAMEGPU_INIT_FUNCTION(generate_population) {
    ...
    const auto ENV_MIN = FLAMEGPU->environment.getProperty<float>("ENV_MIN");
    const auto ENV_MAX = FLAMEGPU->environment.getProperty<float>("ENV_MAX");
...
const CirclesSpatial3DFP32RunData run_circles_spatial3D_fp32(
    ...
    CirclesSpatial3DFP32RunData outputs;
