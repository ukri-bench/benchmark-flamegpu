...
cmake -S . -B build-h100-nvl -DCMAKE_CUDA_ARCHITECTURES="90" -DCMAKE_BUILD_TYPE=Release -DFLAMEGPU_SEATBELTS=OFF -DFLAMEGPU_SHARE_USAGE_STATISTICS=OFF -DCMAKE_SKIP_RPATH=TRUE -DCMAKE_CUDA_HOST_COMPILER=$(which g++)

# Compile the code using all available processors.
cmake --build build-h100-nvl -j `nproc` --verbose
